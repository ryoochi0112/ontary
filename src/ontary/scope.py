...
class ScopePolicy(BaseModel):
    ...
    Resolved 2026-09-04: coverage checks (`ontary.security.covers_scope`)
    use exact-scope-id match as the contract. A consumer scoped at a
    parent level (e.g. company) does not automatically cover objects owned
    by a child scope (e.g. one of that company's teams). Parent-covers-child
    is opt-in and post-1.0; see `covers_scope`'s docstring for the
    enforcement-level detail.
    """
...
class _ScopeReadCache:
    """Per-guarded-read memo for the scope engine's own store reads.
    ...
    def __init__(self) -> None:
        self._current: dict[tuple[str, str], StoredObject | None] = {}
        self._last: dict[tuple[str, str], StoredObject | None] = {}
        self._links_from: dict[tuple[str, str], tuple[str, ...]] = {}
    ...
    def read_current(
        ...
        if key not in self._current:
            self._current[key] = store.read_current(obj_type, obj_id)
    ...
    def read_last(
        ...
        if key not in self._last:
            self._last[key] = store.read_last(obj_type, obj_id)
        return self._last[key]

    def links_from(self, store: Store, link: str, obj_id: str) -> list[str]:
        key = (link, obj_id)
        if key not in self._links_from:
            self._links_from[key] = tuple(store.links_from(link, obj_id))
        return list(self._links_from[key])

    def links_to(self, store: Store, link: str, obj_id: str) -> list[str]:
        key = (link, obj_id)
        if key not in self._links_to:
            self._links_to[key] = tuple(store.links_to(link, obj_id))
        return list(self._links_to[key])

    def links_from_asof(
        self, store: Store, link: str, obj_id: str, asof: str
    ) -> list[str]:
        key = (link, obj_id, asof)
        if key not in self._links_from_asof:
            self._links_from_asof[key] = tuple(
                store.links_from_asof(link, obj_id, asof)
            )
        return list(self._links_from_asof[key])

    def links_to_asof(
        self, store: Store, link: str, obj_id: str, asof: str
    ) -> list[str]:
        key = (link, obj_id, asof)
        if key not in self._links_to_asof:
            self._links_to_asof[key] = tuple(
                store.links_to_asof(link, obj_id, asof)
            )
        return list(self._links_to_asof[key])
    ...
    *,
    ...
    if asof is None:
        obj = (
            ...
            if cache is None
            else cache.read_current(store, obj_type, obj_id)
        )
    else:
        obj = (
            store.read_last(obj_type, obj_id)
            if cache is None
            else cache.read_last(store, obj_type, obj_id)
        )
    ...
    *,
    ...
    if isinstance(rule, ViaLink):
        ...
        if asof is not None:
            if rule.direction == "from":
                parents = (
                    store.links_from_asof(rule.link_api_name, obj_id, asof)
                    if cache is None
                    else cache.links_from_asof(
                        store, rule.link_api_name, obj_id, asof
                    )
                )
            else:
                parents = (
                    store.links_to_asof(rule.link_api_name, obj_id, asof)
                    if cache is None
                    else cache.links_to_asof(
                        store, rule.link_api_name, obj_id, asof
                    )
                )
        elif rule.direction == "from":
            parents = (
                store.links_from(rule.link_api_name, obj_id)
                if cache is None
                else cache.links_from(store, rule.link_api_name, obj_id)
            )
        else:
            parents = (
                store.links_to(rule.link_api_name, obj_id)
                if cache is None
                else cache.links_to(store, rule.link_api_name, obj_id)
            )
