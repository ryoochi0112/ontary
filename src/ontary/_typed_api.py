...
def validate_where_keys(
    ...
    The shape check is repeated here rather than left to `_normalize_where`
    because this branch never reaches it: `set(where)` below runs first, and
    on a non-mapping it does not fail cleanly -- it shredded a string into its
    characters and reported them as unknown FIELD NAMES
    (``where= names unknown field(s) ['0', 'r']``), and raised a bare
    `TypeError` on an `int`. The typed spelling was recorded as bounded by
    `mypy --strict`; that bounds only a literal, not a `where` typed `Any`.
    """
...
def list_objects(
    ...
    *,
    ...
    Omitting limit selects the default bounded Page. Passing limit=None
    explicitly selects the unbounded list form. A positive limit hydrates
    typed page items, and order_by is delegated to the same guarded read path
    as the string form.
    """
...
class _TypedReadMixin:
    ...
    is handed the list `OntologyClient.call_function` reads back to decide
    whether the call released a hidden value. Threaded by reference, exactly
    like `_capability_accesses`: the gate that knows a release happened is in
    `GuardedQuery._aggregate`, and re-deriving "was this hidden and exempt?"
    up here would be a second reader of the same state -- the shape finding 3
    was about.
    """

    _author_dispatch: _AuthorDispatch | None = None
    """Author-provenance grant for this façade's reads -- `None` is the
    fail-closed consumer tier, and NO class overrides it.

    The tier was a `_selection_origin` class attribute that `BoundQuery`
    set to `"author_function"`, which made every hand-constructed
    `BoundQuery` author-tier (finding 4). Provenance is now per-instance
    and engine-minted: `FunctionRegistry.call` sets it on the throwaway
    `BoundQuery` it builds for one declared-Function dispatch, and nothing
    else in the SDK assigns it. A subclass re-declaring this attribute is
    ...
    def _validate_where_keys(
        ...
        if isinstance(cls, str):
            self._query._validate_where_keys(cls, _normalize_where(where))
        else:
    ...
    def aggregate(
        ...
        *,
        ...
        """Ungrouped aggregate over every visible row. Both forms validate
        `where=` keys before delegating; the typed overload additionally validates
        ...
        if isinstance(obj_type, str):
            ...
            return self._query.aggregate(
                ...
                where,
                func=func,
                _author_dispatch=self._author_dispatch,
    ...
    def aggregate_by(
        ...
        *,
        ...
        """Grouped aggregate per distinct `group_by` value. Both forms validate
        `where=` keys before delegating; the typed overload additionally validates
    ...
    def count(
        ...
        """Count only rows visible to this façade's bound consumer."""
        if isinstance(obj_type, str):
            self._validate_where_keys(obj_type, where)
            return self._query.count(self._consumer, obj_type, where)
        api_name = self._api_name_for(obj_type)
        self._validate_where_keys(obj_type, where)
        return self._query.count(self._consumer, api_name, where)
    ...
    def exists(
        ...
        """Probe only rows visible to this façade's bound consumer."""
        if isinstance(obj_type, str):
            self._validate_where_keys(obj_type, where)
            return self._query.exists(self._consumer, obj_type, where)
        api_name = self._api_name_for(obj_type)
        self._validate_where_keys(obj_type, where)
        return self._query.exists(self._consumer, api_name, where)
