...
class WriteRecord(BaseModel):
    ...
    (op, type/id) for a `create`/`update`, or (link_type, from_id, to_id)
    for a `link`. Recorded only for writes that were allowed (authority
    checks happen before the write; refused writes never append)."""

    op: Literal["create", "update", "link"]
