...
ERROR_CODES: dict[str, ErrorCodeInfo] = {
    ...
    "INVALID_PARAMS": ErrorCodeInfo(
        kind="validation",
        description="An action call's parameters failed declared-shape validation.",
    ),
    ...
    "SCOPE_POLICY_ERROR": ErrorCodeInfo(
        kind="validation",
        description=(
            "A ScopePolicy rule references an undeclared object type, link "
            "type, or scope level."
        ),
    ),
    ...
    "INVALID_GROUP_BY": ErrorCodeInfo(
        ...
        description=(
            ...
            "`OntologyClient`'s) `group_by` was falsy (e.g. \"\") -- the "
            "shared aggregation body branches on `group_by`'s truthiness, "
            "so a falsy-but-non-None value would otherwise silently "
            "collapse to the ungrouped path and return a float instead of "
            "a `dict[str, float]`. It is checked in `aggregate_by`, where the "
            "`GuardedQuery`, `BoundQuery`, and client surfaces converge, before "
            "`_aggregate` runs, rather than relying on an assert removed by "
            "`python -O`."
        ),
    ),
