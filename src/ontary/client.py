...
- **reads** -- `get`/`list`/`traverse`/`aggregate`/`aggregate_by`: thin delegation to a
...
class OntologyRuntime:
    ...
    def explain_list(
        ...
        """
        # Run the ordinary path first so where-field and sensitivity refusals
        # are byte-identical to a real list call. Its rows are deliberately
        # unused: explain must also include raw matching rows that path hides.
        self.query.get_objects(consumer, obj_type, where)
        where = self.query._normalize_where_values(obj_type, where)
        selection_trace = _TraceCollector(
            obj_type, "<selection>", self._definition.policy.min_n
        )
        min_n = self.query._selection_min_n(
            consumer, obj_type, where, selection_trace
        )

        traces: list[DecisionTrace] = []
        for row in self._store.read_all(obj_type):
            if where and any(
                row.payload.get(key) != value for key, value in where.items()
            ):
                continue
            traces.append(
                self._explain_row(
...
class OntologyClient(_TypedReadMixin):
    ...
    def list(
        ...
        *,
        ...
        """`limit is None` (the default) is EXACTLY today's list-every-row
        behavior on both the string and typed surfaces (spec pagination-
        hardening AC5). Passing `limit` switches both surfaces to the
        page-filling read (AC3/AC7): the string form returns a `Page` of
        `StoredObject`; the typed form hydrates each `Page.items` entry
        into `T` (same `hydrate` helper as the unpaginated typed path,
        including `redacted_fields`) into a `TypedPage[T]`. `after` is
        passed straight through to `GuardedQuery.get_objects` in EVERY
        case -- including `limit is None` -- so the `ValidationFailed` with
        code `INVALID_CURSOR` propagates unwrapped, never re-wrapped or
        swallowed here, and `after` without `limit` reaches `GuardedQuery`'s
        own `AFTER_WITHOUT_LIMIT` validation refusal
        rather than being silently dropped by this surface never even
        forwarding it (T2 review P1)."""
        return list_objects(
            self._query,
            self._consumer,
            obj_type,
    ...
    @overload
    def traverse(
        self, obj_type: str, link: str, from_id: str
    ) -> builtins.list[StoredObject]: ...
    @overload
    def traverse(
        self, link_cls: LinkHandle[F, T], from_obj_or_id: F | str, /
    ) -> builtins.list[T]: ...
    def traverse(
        self,
        ...
        form is ``traverse(link_cls, from_obj_or_id)``; a typed source may be
        either the source object's model instance or its string id.

        String form: `obj_type` is accepted (rather than inferring it
        purely from the link) so callers get an early, friendly error if
        they name the wrong source type for the link -- `GuardedQuery.
        traverse` itself only needs the link + the source id.

        Typed form: validates the handle belongs to THIS ontology -- its
        `from_cls`/`to_cls` stamps must resolve here (same identity
        discipline as `_api_name_for`/T3), and its `link_api_name` must be
        registered on this client's registry -- else a `ValidationFailed` with
        code `UNKNOWN_NAME`. Each
        returned `StoredObject` is hydrated into `via.to_cls`; an identity-
        revealing link denial for a human consumer raises inside
        `GuardedQuery` before any row returns, unaffected by this check.
        """
        if isinstance(obj_type, LinkHandle):
            source_id = self._traverse_source_id(obj_type, link)
            return self._traverse_via(source_id, obj_type)
        ...
        if link_def.from_type != obj_type:
            raise ValidationFailed(
                f"link {link!r} runs from {link_def.from_type!r}, "
                f"not {obj_type!r}",
                code="UNKNOWN_NAME",
            )
        return self._query.traverse(self._consumer, link, from_id)

    def _traverse_source_id(
        self, link_cls: LinkHandle[F, T], from_obj_or_id: F | str
    ) -> str:
        if isinstance(from_obj_or_id, str):
            return from_obj_or_id
        self._api_name_for(link_cls.from_cls)
        if not isinstance(from_obj_or_id, link_cls.from_cls):
            raise ValidationFailed(
                f"typed traversal source must be an instance of "
                f"{link_cls.from_cls.__name__} or a string id",
                code="INVALID_PARAMS",
            )
        object_def = self._ontology.registry.get_object_type(
            self._api_name_for(link_cls.from_cls)
        )
        source_id = getattr(from_obj_or_id, object_def.primary_key, None)
        if not isinstance(source_id, str):
            raise ValidationFailed(
                f"typed traversal source {link_cls.from_cls.__name__} has no "
                f"string primary-key value for {object_def.primary_key!r}",
                code="INVALID_PARAMS",
            )
        return source_id
    ...
    def call_function(self, api_name: str, params: dict[str, Any]) -> Any:
        ...
        if not audited:
            return self._ontology.functions.call(api_name, bound, params)
