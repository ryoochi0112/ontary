...
- a COUNTING-AND-IDENTITY resolver, asserting each of the ten tools calls
  `resolve_client()` exactly once per invocation, AND (for the six tools
  ...
  registers, not a hardcoded sample an eleventh tool could silently
  escape.
- a RAISING resolver, asserting every one of the ten tools -- both
...
# All ten tools `_register_tools` registers (spec `multi-consumer-mcp.md`
# §4.3), each paired with arguments that let its body run to completion
# against the stub clients below -- covering all ten, not a sample (lesson
...
# it by inspection alone (P1-2: an eleventh tool must not be able to escape
...
# The six tools whose body acts on the resolved client's DATA (as opposed to
...
# `_register_tools`'s own docstring). Only these six can meaningfully be
...
def test_all_tools_matches_what_register_tools_actually_registered() -> None:
    ...
    eleventh tool `_register_tools` grows must show up here as a failure,
...
def test_raising_resolver_fails_closed_without_leaking_message() -> None:
    ...
    # Every one of the ten tools -- unwrapped introspection (`_run_unwrapped`)
