"""FROZEN writer for the `tests/fixtures/upgrade/v0.10.0/` upgrade fixture.
...
`ResolveTicket`, `ArchiveTicket`). Decision A (`v0-10-0-crossing` ledger,
2026-09-04): the tag `v0.10.0` does not exist yet when this fixture is
...
installed from this repo's own local path, not a `git+...@v0.10.0` URL, which
...
the `v0.10.0` tag will point at once it is cut.

**Unlike `write_v0_9_0.py`, this writer never monkeypatches stdlib
`uuid.uuid4`.** `OpenEscalation`'s handler (`examples/tickets/ontology.py`,
frozen application code, unchanged since v0.9.0) still calls
`ctx.insert("Escalation", ...)` with no client-supplied id, but
`ActionContext.insert` (`src/ontary/actions.py`, landed this crossing --
`v0-10-0-crossing/T1`) now fills a missing primary key from the runtime's
configured `id_factory` before `store.insert` is ever called, instead of
falling through to the store's own `uuid.uuid4()` fallback. See
`docs/compatibility.md`'s "Auto-minted ids come from `id_factory` (0.9 ->
0.10)" section for the seam itself. This writer therefore passes
`id_factory=ids` (an `ontary.testing.SequentialIds` instance, the SAME one
every other id in this fixture already comes from) to `Ontology.bind` below,
and that is the ONLY id source it needs.
...
Exercises every 0.10.0 business verb at least once (`EscalateTicket`,
...
Only ever under a v0.10.0 `ontary` venv installed from THIS repo's own local
...
THIS repo's own root via `PYTHONPATH` -- from this repo's root:
    ...
    PYTHONPATH=/path/to/this/repo /path/to/scratch/.venv/bin/python \\
        tests/fixtures/upgrade/writers/write_v0_10_0.py

Installing from the local path (never editable) is what makes
`ontary.__version__` report real installed metadata (`"0.10.0"`) instead of the
...
(`../v0.10.0/tickets.sqlite`, `../v0.10.0/expected.json`, resolved from
...
`v1-acceptance-gate` §6 "AC4"): it is written against the v0.10.0 API on
...
proves THIS SCRIPT, re-run unmodified once the `v0.10.0` tag exists, still
...
#: of it.
_INSTANT = datetime(2026, 9, 4, 9, 0, tzinfo=timezone.utc)


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(f"write_v0_10_0.py: {message}")


def _check_running_under_v0_10_0() -> None:
    ...
    """
    _require(
        ontary.__version__ == "0.10.0",
        "must run under the v0.10.0 ontary venv (see this module's docstring) -- "
...
def _check_ontary_did_not_resolve_from_this_repos_own_src() -> None:
    ...
    `ontary.__version__ == "0.10.0"` once `pyproject.toml` is bumped, but lands
    ...
    _require(
        ...
        "v0.10.0 build. Re-run with --python pointed at a venv built from "
        "`uv pip install <this-repo-path>` (see this module's docstring)",
    )


def _check_examples_tickets_is_the_v0_10_0_shape(ontology: Any) -> None:
    """Guards the OTHER half of a wrong-venv run: `PYTHONPATH` pointing at
    the wrong tree while `ontary` itself is correctly the v0.10.0 install.
    `examples.tickets` at v0.10.0 declares exactly 6 object types (the
    `Escalation` addition made at the 0.9.0 crossing, `d38ac56`) and a
    `Ticket` at version 2 with a `channel` property.
    """
    ...
    _require(
        ...
        "examples.tickets resolved to a PRE-v0.10.0 Ticket shape "
...
#: genuinely empty, unlike `write_v0_7_0.py`/`write_v0_8_0.py`'s seven-item
#: `covered` list. Kept as a named constant (rather than inlined) so
...
def main() -> None:  # noqa: C901 -- a straight-line seed script, not a decision tree
    _check_running_under_v0_10_0()
    _check_ontary_did_not_resolve_from_this_repos_own_src()

    # Unlike `write_v0_9_0.py`, no `uuid.uuid4` monkeypatch: `OpenEscalation`'s
    # handler calls `ctx.insert("Escalation", ...)` with no client-supplied id,
    # and `ActionContext.insert` (`src/ontary/actions.py`, landed this
    # crossing -- `v0-10-0-crossing/T1`; see `docs/compatibility.md`'s
    # "Auto-minted ids come from `id_factory` (0.9 -> 0.10)" section) now
    # fills that id from the runtime's `id_factory` before
    # the store is ever touched, so the SAME `ids = SequentialIds(...)` bound
    # to `Ontology.bind` below (rather than a private `uuid.uuid4` patch) is
    # the ONLY id source this fixture needs.
    ontology, _unused_memory_store = build_ontology()
    _check_examples_tickets_is_the_v0_10_0_shape(ontology)

    _FIXTURE_DIR.mkdir(parents=True, exist_ok=True)
    for suffix in ("", "-journal", "-wal", "-shm"):
        _STORE_PATH.with_name(_STORE_PATH.name + suffix).unlink(missing_ok=True)

    ids = SequentialIds("v0100")
    ...
    #    directly (v0.10.0's `Ticket` is already at version 2 -- there is no
    ...
    # -- every 0.10.0 business verb, at least once, so the fixture records a
    #    full-surface 0.10.0 store: `EscalateTicket` (the one v0.8.0-era verb,
    #    with its inline-dispatched effect), then the `Escalation`
    #    lifecycle (added at the 0.9.0 crossing) -- `OpenEscalation` ->
    #    `ResolveTicket` -> `ArchiveTicket`
    ...
    try:
        ...
        raise SystemExit(
            "write_v0_10_0.py: expected queue_b's ticketStats call to refuse "
    ...
    # Already at the current shape (v0.10.0's own Ticket is version 2 with
    ...
    expected: dict[str, Any] = {
        ...
        "tickets": {
            "note": (
                "`as_written` IS `expected_after_upgrade` here: v0.10.0's own "
