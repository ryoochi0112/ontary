...
def test_explain_list_snapshots_where_once_and_preserves_result(
    make_registry: RegistryFactory,
    make_policy: PolicyFactory,
) -> None:
    """The old suite could not build the stateful input needed to observe
    duplicate normalization. A counting dict subclass proves the caller's
    mapping is read once, while the result comparison pins explain semantics.
    """

    client, store = _build_client(_library_librarian(), make_registry, make_policy)
    store.insert(
        "Book",
        {"id": "book-2", "shelf_id": "shelf-1", "title": "Different", "pages": 200},
        SRC,
    )
    runtime = OntologyRuntime(client._ontology, store)
    expected = runtime.explain_list(
        _library_librarian(), "Book", where={"title": "Foundry"}
    )

    class _CountingWhere(dict[str, Any]):
        item_calls = 0
    ...
    where = _CountingWhere(title="Foundry")
    actual = runtime.explain_list(_library_librarian(), "Book", where=where)

    assert where.item_calls == 1
    assert [
        (trace.object_id, trace.verdict, trace.min_n.outcome, trace.min_n.count)
        for trace in actual
    ] == [("book-1", "visible", "passed", 1)]
    assert actual == expected


@pytest.mark.parametrize(
    ("consumer", "obj_type", "where", "incoherent_scope"),
    [
        pytest.param(
            _library_librarian(),
            "NoSuchType",
        ...
        pytest.param(
            ...
            {"not_a_field": "Foundry"},
            False,
            id="unknown-where-field",
        ),
        pytest.param(
            _library_librarian(),
            "Book",
            {f"unknown_{index}": index for index in range(21)},
            False,
            id="many-unknown-where-fields",
        ),
        pytest.param(
            _ai_librarian(),
            "Book",
            {"internal_note": "shhh"},
            False,
            id="hidden-field-bare-eq",
        ),
        pytest.param(
            _ai_librarian(),
            "Book",
            {"internal_note": {"contains": "sh"}},
            False,
            id="hidden-field-learned-operator",
        ),
        pytest.param(
            _library_librarian(),
            "Book",
        ...
        pytest.param(
            _library_librarian(),
            "Book",
            {"title": {"eq": "Foundry", "ne": "Other"}},
            False,
            id="malformed-multi-key-condition",
        ),
        pytest.param(
            _library_librarian(),
            "Book",
        ...
        pytest.param(
            ...
            {"title": "Foundry"},
            True,
            id="incoherent-scope-policy",
        ),
    ],
)
def test_explain_list_refusals_match_get_objects(
    consumer: Consumer,
    obj_type: str,
    where: Any,
    incoherent_scope: bool,
    make_registry: RegistryFactory,
    make_policy: PolicyFactory,
) -> None:
    """The old suite could not construct this paired refusal matrix: it had
    no operator-only `explain_list` cases for hidden fields, malformed
    conditions, non-string keys, or a 21-field mapping. Compare the exact
    exception type, code, and message against the real read path so a gate
    added to only `get_objects` cannot hide drift.
    """

    client, store = _build_client(_library_librarian(), make_registry, make_policy)
    if incoherent_scope:
        client._ontology.policy.unscoped_types.add("Book")
    runtime = OntologyRuntime(client._ontology, store)

    def refusal_signature(call: Callable[[], Any]) -> tuple[type[BaseException], Any, str]:
        with pytest.raises(Exception) as excinfo:
            call()
        error = excinfo.value
        return type(error), getattr(error, "code", None), str(error)

    explained = refusal_signature(
        lambda: runtime.explain_list(consumer, obj_type, where)
    )
    ordinary = refusal_signature(
        lambda: runtime.query.get_objects(
            consumer, obj_type, where, limit=None, order_by=None
        )
    )

    assert explained == ordinary
