...
def test_storage_envelope_doc_ceiling_matches_published_constants() -> None:
    ...
    silently drift from the code a `Finding` message actually quotes (L23:
    nothing here asserts a millisecond/timing figure -- only the row count
    and label each backend's frozen record carries). Equality, not a subset
    check, so a backend added to one side and forgotten on the other reds
    either way.
    """
...
def test_storage_envelope_postgres_ceiling_is_derived_from_the_published_curves() -> None:
    ...
    constant, the doc's summary ceiling row, and this file's own exact-value
    pin) and a human catch on the job log was the only thing that ever
    caught a wrong-but-self-consistent triple (E5b's proven escape:
    `rows=6_400`, twice the then-published ceiling, shipped green). This
    test replaces that with ONE generating rule: parse every Postgres run
    table `docs/storage.md` publishes (each `#### Run <id>` section is one
    CI job's raw evidence), compute the worst -- i.e. SLOWEST -- per-row
    rate across every row and both linear read classes (`aggregate`,
    narrow-scope; the bounded-page column is flat and carries no ceiling,
    per this section's own prose), and assert the published
    `STORAGE_ENVELOPE["PostgresStore"].rows` sits at or below the
    one-second crossing that rate implies. Paste a slower run 3's table in
    and this assertion re-derives itself without touching this file (S2:
    not another hand-copied constant -- see `diagnose.py`'s STORAGE_ENVELOPE
    comment for the full site count across both files).
    ...
    run's window; and the document-wide count of well-formed cost-table
    blocks must equal the number of matched `#### Run` headings plus one
    (the `ObjectStore` table above them), so a run's table cannot vanish
    from the parse by demoting its own heading to a different level while
    the table itself stays intact and visible to a human reader. Each run's
    window is bounded at the next markdown heading of any level, or at EOF
    if the run's heading is the last one in the document.
    """
    storage = _DOCS / "storage.md"
    text = storage.read_text()
    ...
    parsed_run_ids = {int(heading.group("run_id")) for heading in run_headings}
    ...
    # human reader. Catch that with a document-wide census: every
    # `table_header`-shaped block in storage.md must be accounted for by
    # exactly one run heading, plus the one `ObjectStore`/SQLite table that
    # precedes all the run tables.
    doc_wide_tables = list(re.finditer(table_header, text))
    assert len(doc_wide_tables) == len(run_headings) + 1, (
        f"storage.md has {len(doc_wide_tables)} Postgres-cost-table-shaped "
        f"blocks (the `| Rows | Bounded page ... |` shape) but "
        f"{len(run_headings)} `#### Run` headings -- expected exactly "
        f"{len(run_headings) + 1} (one per run heading, plus the "
        "`ObjectStore`/SQLite table above them). A run's table can go "
        "missing from this parse without ever failing an assertion inside "
        "the per-run loop below if its own `#### Run` heading stops "
