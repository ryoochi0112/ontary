...
def test_typed_list_returns_list_of_ticket() -> None:
    ...
    assert {t.subject for t in tickets} == {
        "Account locked",
        "Cancel subscription",
        "Card declined",
        "Duplicate invoice",
...
def test_omitted_list_limit_returns_bounded_typed_pages() -> None:
    client, ids = _client()
    typed = client.list(Ticket, where={"queue_id": ids["queue_a_id"]})
    assert_type(typed, TypedPage[Ticket])
    assert [ticket.id for ticket in typed.items] == [
        ids[f"ticket_{number}_id"] for number in (1, 2, 4, 5, 6, 7, 8, 9, 10)
    ]

    dynamic = client.list("Ticket", where={"queue_id": ids["queue_a_id"]})
    assert_type(dynamic, Page)
    assert [row.payload["id"] for row in dynamic.items] == [
        ids[f"ticket_{number}_id"] for number in (1, 2, 4, 5, 6, 7, 8, 9, 10)
    ]
...
def test_typed_reverse_traverse_returns_list_of_source_type() -> None:
    client, ids = _client()
    tickets = client.traverse(ticketInQueue, ids["queue_a_id"], reverse=True)
    assert_type(tickets, list[Ticket])
...
def test_typed_reverse_traverse_dynamic_flag_returns_union() -> None:
    client, ids = _client()
    reverse: bool = True
    rows = client.traverse(ticketInQueue, ids["queue_a_id"], reverse=reverse)
...
def test_bound_query_typed_reverse_traverse_returns_list_of_source_type() -> None:
    ontology, store = build_ontology()
    ids = load_fixtures(store)
    consumer = Consumer(
        actor_id="agent-1",
    ...
    tickets = bound.traverse(ticketInQueue, ids["queue_a_id"], reverse=True)
    assert_type(tickets, list[Ticket])
    assert {ticket.id for ticket in tickets} == {
        ids[f"ticket_{number}_id"] for number in (1, 2, 4, 5, 6, 7, 8, 9, 10)
    }
...
def test_bound_query_typed_reverse_dynamic_flag_returns_union() -> None:
    ontology, store = build_ontology()
    ids = load_fixtures(store)
    consumer = Consumer(
        actor_id="agent-1",
    ...
    rows = bound.traverse(ticketInQueue, ids["queue_a_id"], reverse=reverse)
    assert_type(rows, list[Ticket] | list[Queue])
    assert len(rows) == 9
