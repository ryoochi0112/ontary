...
_COMPLETED_MAJOR_LAST_MINOR: dict[int, int] = {
    0: 9,  # Hand-edited release record: 0.x's last completed minor is v0.9.0.
}
...
def _current_ticket_payloads(
    store_dump: dict[str, Any], *, upcast_expected: bool
) -> dict[str, dict[str, Any]]:
    """`upcast_expected` is derived by the caller from the SAME fixture's own
    `expected.json` (`tickets.as_written != tickets.expected_after_upgrade`),
    never from the tag string -- a widened rule (S2), not a per-tag branch.
    ...
    for row in store_dump["tables"]["objects"]:
        ...
        if upcast_expected:
            assert row["type_version"] == 1, "frozen fixtures must contain v1 Ticket rows"
...
def test_ticket_fixture_opens_silently_and_matches_expected(tag: str, tmp_path: Path) -> None:
    ...
    assert (
        _current_ticket_payloads(as_written_dump, upcast_expected=ticket_upcast_expected)
        == expected["tickets"]["as_written"]
    )
