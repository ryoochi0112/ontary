...
def _base_library_ontology(*, min_n: int = 1) -> tuple[Ontology, type[OntologyObject]]:
    ...
    def _count_books_on_shelf(query: BoundQuery, params: dict[str, Any]) -> int:
        return len(query.list("Book", where={"shelf_id": params["shelf_id"]}))
...
def _build_server(consumer: Consumer) -> tuple[FastMCP, ObjectStore]:
...
def test_query_objects_description_and_schema_state_where_and_pagination_contract() -> None:
    ...
    for phrase in (
        "equality-only matching on payload fields",
    ...
    assert "Equality-only matching on payload fields" in where_description
