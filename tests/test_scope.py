...
def _library_policy(make_policy: PolicyFactory) -> ScopePolicy:
    return make_policy(
        ...
        rules={
            ...
            "Reservation": [
                ViaLink(link_api_name="reservedFor", direction="to", parent_type="Book"),
            ],
        },
        min_n=3,
    )
...
def test_self_scope_resolves_directly(
    ...
    resolved = resolve_owning_scope(_library_policy(make_policy), store, "Library", "lib-1")
...
def test_via_link_from_direction_resolves_parent(
    ...
    resolved = resolve_owning_scope(_library_policy(make_policy), store, "Shelf", "shelf-1")
...
def test_include_retired_default_is_off_and_a_retired_row_resolves_nothing(
    ...
    assert resolve_owning_scope(policy, store, "Shelf", "shelf-1", include_retired=True) == {
        "shelf": "shelf-1",
        "library": "lib-1",
    }
...
def test_direct_property_shortcut_resolves_without_link(
    ...
    resolved = resolve_owning_scope(_library_policy(make_policy), store, "Book", "book-1")
...
def test_multi_hop_chain_grandchild_to_child_to_scope_object(
    ...
    resolved = resolve_owning_scope(_library_policy(make_policy), store, "Loan", "loan-1")
    assert resolved == {"shelf": "shelf-1", "library": "lib-1"}
...
def test_via_link_to_direction_resolves_from_id_parent(
    ...
    resolved = resolve_owning_scope(_library_policy(make_policy), store, "Reservation", "res-1")
    assert resolved == {"shelf": "shelf-1", "library": "lib-1"}
...
def test_scope_read_cache_separates_current_from_last(
    make_registry: RegistryFactory,
    make_store: StoreFactory,
) -> None:
    """Collapsing current/last makes the retired row disappear from last."""
    registry = _library_registry(make_registry)
    store = make_store(registry)
    store.insert("Shelf", {"id": "shelf-retired"}, SRC)
    store.retire_object("Shelf", "shelf-retired")
    cache = _ScopeReadCache()

    assert cache.read_current(store, "Shelf", "shelf-retired") is None
    last = cache.read_last(store, "Shelf", "shelf-retired")
    assert last is not None
    assert last.payload == {"id": "shelf-retired"}


def test_scope_read_cache_separates_every_method_and_argument() -> None:
    """Dropping any operation or argument from a cache key returns the echo
    from an earlier, different read instead of the requested full tuple."""

    class ScopeReadProbe:
        def __getattr__(self, operation: str) -> Callable[..., tuple[str, ...]]:
            def echo(*args: str) -> tuple[str, ...]:
                return (operation, *args)

            return echo

    cache = _ScopeReadCache()
    store = ScopeReadProbe()
    cache_methods = {
        name: method
        for name, method in inspect.getmembers(_ScopeReadCache, predicate=inspect.isfunction)
        if not name.startswith("_")
    }

    for operation, method in cache_methods.items():
        parameter_names = tuple(inspect.signature(method).parameters)[2:]
        baseline = tuple(f"argument-{index}-a" for index in range(len(parameter_names)))
        answer = method(cache, store, *baseline)
        assert tuple(answer) == (operation, *baseline)

        for index in range(len(parameter_names)):
            varied = list(baseline)
            varied[index] = f"argument-{index}-b"
            answer = method(cache, store, *varied)
            assert tuple(answer) == (operation, *varied)


def test_custom_resolver_keeps_raw_store_and_raw_call_count_with_cache(
    make_registry: RegistryFactory,
    make_store: StoreFactory,
) -> None:
    """Passing the memo to author code changes the observed store identity."""
    registry = _library_registry(make_registry)
    store = make_store(registry)
    store.insert("Library", {"id": "lib-1"}, SRC)
    observed_stores: list[Store] = []

    def resolving_shelf(author_store: Store, obj_type: str, obj_id: str) -> str | None:
        observed_stores.append(author_store)
        row = author_store.read_current(obj_type, obj_id)
        return "shelf-special" if row is not None else None

    policy = ScopePolicy(
        levels=LEVELS,
        rules={"Library": [CustomResolver(level="shelf", fn=resolving_shelf)]},
    )

    uncached = resolve_owning_scope(policy, store, "Library", "lib-1")
    uncached_call_count = len(observed_stores)
    cached = resolve_owning_scope(
        policy,
        store,
        "Library",
        "lib-1",
    ...
    cached_call_count = len(observed_stores) - uncached_call_count

    assert uncached == cached == {"shelf": "shelf-special", "library": None}
    assert uncached_call_count == cached_call_count == 2
    assert observed_stores == [store, store, store, store]
...
def test_nonexistent_object_resolves_to_none(
    ...
    registry = _library_registry(make_registry)
    store = make_store(registry)
    resolved = resolve_owning_scope(_library_policy(make_policy), store, "Book", "no-such-book")
    assert resolved == {"shelf": None, "library": None}
...
def test_cycle_resolves_to_none_never_infinite_recursion(
    ...
    policy = ScopePolicy(
        ...
        rules={
            "Shelf": [
                ViaLink(link_api_name="shelfLoop", direction="from", parent_type="Shelf"),
            ],
        },
    )
...
def test_validate_rejects_undeclared_link(
    ...
    policy = ScopePolicy(
        levels=LEVELS,
        rules={
            "Book": [ViaLink(link_api_name="noSuchLink", direction="from", parent_type="Shelf")]
        },
    )
...
def test_validate_rejects_direct_property_undeclared_property(
    ...
    policy = ScopePolicy(
        levels=LEVELS,
        rules={"Book": [DirectProperty(level="shelf", property_name="no_such_property")]},
    )
    with raises_code(ValidationFailed, "SCOPE_POLICY_ERROR") as exc_info:
        policy.validate(registry)
...
def test_validate_rejects_direction_mismatch(
    ...
    policy = ScopePolicy(
        levels=LEVELS,
        rules={"Book": [ViaLink(link_api_name="onShelf", direction="to", parent_type="Shelf")]},
    )
    with raises_code(ValidationFailed, "SCOPE_POLICY_ERROR") as exc_info:
        policy.validate(registry)
...
def test_covers_scope_broader_consumer_level_checks_its_own_level_only(
    ...
    assert covers_scope(_library_policy(make_policy), consumer, resolved_other_library) is False
