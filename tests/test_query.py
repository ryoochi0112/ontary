...
def _operator_query(
    ...
    policy = make_policy(
        levels=["team"], unscoped_types={"Value"}, min_n=1
    )
...
@pytest.mark.parametrize(
    ("operator", "field", "operand", "expected_ids"), _VALID_OPERATOR_CASES
)
...
def test_scope_key_field_exempt_from_where_gate_but_still_redacted(
    ...
    consumer = _human("shelf", "shelf-1")
    results = gq.get_objects(
        consumer, "Book", where={"shelf_id": "shelf-1"}, limit=None
    )
    assert [r.payload["id"] for r in results] == ["book-1"]
...
def test_where_on_hidden_field_denied_for_human_allowed_for_ai(
    ...
    with raises_code(VisibilityError, "VISIBILITY_DENIED"):
        gq.get_objects(
            human, "Book", limit=None, order_by="acquisition_cost"
        )

    # AI is permitted (acquisition_cost is ai_usable) -- no raise.
    results = gq.get_objects(
        ai, "Book", where={"acquisition_cost": 42.0}, limit=None
    )
    assert [r.payload["id"] for r in results] == ["book-1"]
...
def test_unknown_where_key_is_rejected_before_non_match(
    ...
    assert str(excinfo.value) == (
        "Book: order_by names unknown field(s) ['ratng']"
    )
...
def test_identity_revealing_traverse_denied_before_target_resolution(
    ...
    monkeypatch.setattr(store, "links_to" if reverse else "links_from", fail_if_target_lookup_is_attempted)
    with raises_code(VisibilityError, "VISIBILITY_DENIED"):
...
def _hide_record_b_group_id(registry: OntologyRegistry) -> None:
    ...
    group_id = next(
        prop for prop in obj_def.properties if prop.name == "group_id"
    )
...
def test_scope_key_exemption_does_not_pool_across_types(
    ...
    results = gq.get_objects(
        person_consumer, "TeamA", where={"person_id": "p-1"}, limit=None
    )
...
def test_order_by_hidden_scope_key_denied_before_rank_is_observable(
    ...
    try:
        rows = gq.get_objects(
            consumer, "RecordB", limit=None, order_by=order_by
        )
...
def test_aggregate_by_hidden_scope_key_denied_without_returning_group_keys(
    ...
    denial: VisibilityError | None = None
    try:
        returned_groups = gq.aggregate_by(
            consumer, "RecordB", "score", "group_id"
        )
    except VisibilityError as exc:
        denial = exc
...
def test_get_objects_redacts_hidden_scope_key_from_returned_payload(
    ...
    store = make_store(registry)
    store.insert(
        "RecordB", {"id": "b-1", "group_id": "secret-group"}, SRC
    )
    gq = GuardedQuery(store, registry, _shared_field_policy(make_policy))

    rows = gq.get_objects(
        _human("group", "secret-group"), "RecordB", limit=None
    )
...
def test_contains_alphabet_walk_cannot_recover_hidden_scope_key(
    ...
    store.insert(
        "RecordB", {"id": "target", "org_id": ORG, "group_id": hidden_value}, SRC
    )
    policy = _shared_field_policy(make_policy)
    gq = GuardedQuery(store, registry, policy)
...
def test_two_faced_where_mapping_cannot_split_the_gate_from_the_matcher(
    ...
    """
    gq, consumer = _walkable_record_b(
        make_registry, make_policy, make_store, "amy-4402"
    )
...
def test_two_faced_condition_cannot_split_the_gate_from_the_matcher(
    ...
    rows = gq.get_objects(
        consumer, "RecordB", where={"group_id": condition}, limit=None
    )
...
def test_a_wrongly_typed_bare_operand_never_preempts_the_visibility_denial(
    ...
    store = make_store(registry)
    store.insert(
        "RecordB", {"id": "keyed", "org_id": ORG, "group_id": "secret-group"}, SRC
    )
    policy = _shared_field_policy(make_policy)
    gq = GuardedQuery(store, registry, policy)
    consumer = _human("org", ORG)
...
def test_in_over_a_list_containing_null_is_not_supply_shaped(
    ...
    with raises_code(VisibilityError, "VISIBILITY_DENIED"):
        gq.get_objects(
            consumer, "RecordB", where={"group_id": {"in": [None]}}, limit=None
        )
    with raises_code(VisibilityError, "VISIBILITY_DENIED"):
        gq.get_objects(
            consumer, "RecordB", where={"group_id": {"in": []}}, limit=None
        )
...
def test_supplied_scalar_keeps_the_scope_key_exemption(
    ...
    assert [
        row.payload["id"]
        for row in gq.get_objects(
            consumer, "RecordB", where={"group_id": "amy-4402"}, limit=None
        )
    ] == ["target"]
    assert [
        row.payload["id"]
...
def test_bare_null_still_filters_a_field_that_is_not_hidden(
    ...
    rows = gq.get_objects(
        _human("org", ORG), "TeamA", where={"person_id": None}, limit=None
    )
...
def test_non_id_primary_key_works_across_all_read_paths(
    ...
    assert [
        r.payload["code"]
        for r in gq.get_objects(consumer, "Item", limit=None)
    ] == ["item-1"]
...
DISCLOSURE_REFERENCE_SHAPES: tuple[
...
def test_released_reads_cannot_recover_an_individual_hidden_value(
    ...
    _seed_two_shelves(store)
    values = DISCLOSURE_SEEDED_VALUES.get(
        shape_name, (1.0, 4.0, 7.0, 12.0, 20.0, 31.0)
    )
    seeded = tuple(
        (f"r{i}", value, "a" if i < 3 else "b")
    ...
    for read in reads:
        ...
        if group_by is None:
            visible_probe = query.aggregate(
                human, "Reading", "score", where=where, func=func
            )
...
def test_every_public_read_routes_through_the_disclosure_gate(
    ...
    registry = _contributor_registry(make_registry)
    query = GuardedQuery(
        make_store(registry), registry, _contributor_policy(make_policy)
    )
    human = _human("shelf", "shelf-1")
    invokers: dict[str, Callable[[GuardedQuery], object]] = {
        "get_objects": lambda guarded: guarded.get_objects(
            human, "Reading", limit=None
        ),
        ...
        "traverse": lambda guarded: guarded.traverse(
            human, "inLibrary", "shelf-1"
        ),
        "aggregate": lambda guarded: guarded.aggregate(
            human, "Reading", "score"
        ),
        "aggregate_by": lambda guarded: guarded.aggregate_by(
            human, "Reading", "score", "shelf_id"
        ),
        "count_contributors": lambda guarded: guarded.count_contributors(
            human, "Reading"
        ),
    }
    public_reads = {
    ...
    monkeypatch.setattr(
        GuardedQuery, "_require_coherent_scope", stop_at_disclosure_gate
    )
...
def test_count_only_redaction_switch_stays_private_to_guarded_query(
    ...
    """The count projection switch cannot escape the guarded engine.
    ...
    payloads. The signature check catches that structural drift; the calls
    below prove the three list entry points reject the keyword at runtime.
    """
    registry = _library_registry(make_registry)
    store = make_store(registry)
    _seed_two_shelves(store)
    ...
    rejecting_calls: tuple[tuple[str, Callable[[], object]], ...] = (
        (
        ...
        (
            "OntologyClient.list",
            lambda: client.list(
                "Book", limit=None, _redact_rows=False  # type: ignore[call-arg]
            ),
        ),
        (
            "BoundQuery.list",
    ...
    for _surface, invoke in rejecting_calls:
        with pytest.raises(TypeError, match="_redact_rows"):
            invoke()
...
def test_aggregate_min_n_survives_contributor_link_closure(
    ...
    for i, value in enumerate((10.0, 20.0, 30.0)):
        store.insert(
            "Reading", {"id": f"r{i}", "shelf_id": "shelf-1", "score": value}, SRC
        )
...
def test_retire_action_does_not_release_an_individuals_hidden_values(
    ...
    registry.register_action_type(
        ActionTypeDef(
            ...
            parameters=[
                ActionParameterDef(name="reader_id", type="str", required=True)
            ],
        )
    )
    store = make_store(registry)
    ...
    def retire_reader(
        ctx: ActionContext, params: RetireReaderParams
    ) -> dict[str, str]:
...
def test_two_ontologies_coexist_without_cross_talk(
    ...
    consumer = _human("shelf", "shelf-1")
    assert [
        r.payload["id"] for r in gq_a.get_objects(consumer, "Book", limit=None)
    ] == ["book-1"]
    assert [
        r.payload["id"] for r in gq_b.get_objects(consumer, "Book", limit=None)
    ] == ["book-9"]
...
def test_row_visibility_filters_get_objects(
    ...
    assert {
        r.payload["id"] for r in gq.get_objects(human, "Book", limit=None)
    } == {"book-free"}
    assert {
        r.payload["id"] for r in gq.get_objects(ai, "Book", limit=None)
    } == {"book-free", "book-loaned"}
...
def test_aggregate_value_field_gate_ignores_scope_key_exemption(
    ...
    results = gq.get_objects(
        consumer, "RecordB", where={"group_id": "secret-group"}, limit=None
    )
...
def test_supply_shaped_where_honors_scope_key_exemption_on_both_gates(
    ...
    rows = gq.get_objects(
        consumer, "RecordB", where=where, limit=None
    )
    result = gq.aggregate(
        consumer,
        "RecordB",
    ...
    contributor_count = gq.count_contributors(
        consumer, "RecordB", where=where
    )
...
def test_aggregate_hidden_field_exempt_when_contributor_dedup_declared(
    ...
    result = gq.aggregate(
        human, "Reading", "raw_score", _author_dispatch=_AUTHOR_DISPATCH
    )
    assert result == pytest.approx(4.0)
...
def test_contributor_exemption_releases_hidden_mean_and_count(
    ...
    with raises_code(VisibilityError, "VISIBILITY_DENIED"):
        query.aggregate_by(
            human, "Reading", "raw_score", "shelf_id", func=func
        )
...
def test_asserting_author_provenance_as_a_value_does_not_grant_it(
    ...
    for claimed in ("author_function", True, object(), _AuthorDispatch()):
        with raises_code(VisibilityError, "VISIBILITY_DENIED"):
            query.aggregate(
                human, "Reading", "raw_score", _author_dispatch=claimed
            )
...
def test_contributor_type_visible_field_keeps_every_aggregate_function(
    ...
    query, human, _ = _distinct_contributor_aggregate_query(
        make_registry, make_policy, make_store
    )

    assert query.aggregate(
        human, "Reading", "score", func=func
