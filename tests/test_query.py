...
# -- AC11 companion: memo hits at a level/population the family above never
# -- reaches --------------------------------------------------------------
#
# Every case above seeds ONE row per parent and a consumer fixed at the
# "shelf" level. `_ScopeReadCache` (`ontary/scope.py`) memoizes by
# `(type, id)` and `(link, id)`; `resolve_owning_scope` resolves EVERY level
# in `policy.levels` for a row regardless of which level the consumer is
# actually at (`covers_scope` only ever looks at the one it needs), and a
# single row's OWN keys are consulted once per level, so a memo HIT --
# the SAME key read back a second time, as opposed to written once and
# never read again -- only starts happening once either (a) a row is
# resolved past the first level, reusing what an earlier level already
# cached, or (b) a second row shares a parent with one already resolved,
# reusing what that first row's resolution wrote. A hit that is never
# exercised can be answered wrong for free: a defect confined to a level no
# seeded consumer checks, or to a shared-parent read no population ever
# repeats, ships green today. `level` below ranges over every level in
# `LEVELS` (not just "shelf"), and the population always gives two Books
# one shared Shelf, so the second sibling's resolution reads back what the
# first one wrote.
_HIT_IDENTITY_CONSUMER_SCOPE_ID = {"shelf": "shelf-1", "library": "lib-1"}
_HIT_IDENTITY_RULE_SHAPES = ("via-link", "direct-property")


def _hit_identity_rules(rule_shape: str) -> dict[str, list[object]]:
    """The Shelf -> Library hop, in one of two mutually exclusive shapes.

    ``"via-link"`` is the family's own shape above (`ViaLink` the whole way
    up) and is what exercises the cache's `links_from` dispatch: a hit that
    answers empty, or answers with a DIFFERENT key's link list, changes
    which parent id the climb continues from. ``"direct-property"`` swaps
    the SAME hop for a `DirectProperty` read of Shelf's own (already
    declared, otherwise-unused) `library_id` column instead. That is the
    only rule kind that consults the resolved OBJECT's payload rather than
    just its id: `SelfScope` answers from `obj_id` alone and `ViaLink`
    answers from the link read alone, so a `read_current` hit that hands
    back a DIFFERENT key's object -- still non-`None`, just the wrong row --
    is invisible to either of them and only `DirectProperty` can notice.
    The two shapes are never combined on Shelf: a `ViaLink` fallback after
    a failing `DirectProperty` would silently re-derive the right answer
    over the link and mask the very corruption this shape exists to catch.
    """
    library_rule: object = (
        ViaLink(link_api_name="inLibrary", direction="from", parent_type="Library")
        if rule_shape == "via-link"
        else DirectProperty(level="library", property_name="library_id")
    )
    return {
        "Library": [SelfScope(level="library")],
        "Shelf": [SelfScope(level="shelf"), library_rule],
        "Book": [ViaLink(link_api_name="onShelf", direction="from", parent_type="Shelf")],
    }


def _seed_shared_parent_siblings(store: Store) -> None:
    """Two Books sharing one Shelf, itself in one Library.

    Distinguishable, non-empty ratings on both -- 4.0 and 2.0, never 0 or
    absent -- so a cache that answered every read with `[]`/`None` on both
    the cached and uncached run could not pass the non-degeneracy pin below
    by accident.
    """
    store.insert("Library", {"id": "lib-1"}, SRC)
    store.insert("Shelf", {"id": "shelf-1", "note": "fiction", "library_id": "lib-1"}, SRC)
    store.create_link("inLibrary", "shelf-1", "lib-1")
    store.insert("Book", {"id": "book-target", "rating": 4.0}, SRC)
    store.create_link("onShelf", "book-target", "shelf-1")
    store.insert("Book", {"id": "book-second", "rating": 2.0}, SRC)
    store.create_link("onShelf", "book-second", "shelf-1")


@pytest.mark.parametrize(
    ("level", "rule_shape"),
    list(product(LEVELS, _HIT_IDENTITY_RULE_SHAPES)),
    ids=[f"{level}-{shape}" for level, shape in product(LEVELS, _HIT_IDENTITY_RULE_SHAPES)],
)
def test_scope_cache_hit_identity_at_every_level_with_shared_parents(
    level: str,
    rule_shape: str,
    make_registry: RegistryFactory,
    make_policy: PolicyFactory,
    ...
    """Companion to the byte-identity family above: same oracle, but at a
    level -- and over a population -- that actually makes the memo HIT."""
    registry = _library_registry(make_registry)
    store = make_store(registry)
    _seed_shared_parent_siblings(store)
    target_id = "book-target"

    policy = make_policy(
        levels=LEVELS,
        rules=_hit_identity_rules(rule_shape),
        row_visibility={},
        min_n=1,
    )
    query = GuardedQuery(store, registry, policy)
    consumer = _human(level, _HIT_IDENTITY_CONSUMER_SCOPE_ID[level])
    ...
    assert cached == uncached
    # Not degenerate: both siblings are visible, with distinguishable
    # ratings, at every level/shape cell -- never merely both-empty.
    unbounded = json.loads(cached["get_objects_unbounded"])["ok"]
    assert {row["payload"]["id"]: row["payload"]["rating"] for row in unbounded} == {
        "book-target": 4.0,
        "book-second": 2.0,
    }
    assert json.loads(cached["aggregate"])["ok"] == pytest.approx(3.0)
