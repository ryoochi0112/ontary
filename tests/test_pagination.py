...
from ontary.query import DEFAULT_READ_LIMIT, GuardedQuery, Page, TypedPage
...
def test_cursor_round_trip_matches_unpaginated_no_gaps_no_repeats(
    ...
    unpaginated = gq.get_objects(consumer, "Widget", limit=None)
...
def test_limit_none_returns_exactly_todays_list_type_and_contents(
    ...
    result = gq.get_objects(_narrow(), "Widget", limit=None)
...
def test_omitted_limit_is_the_bounded_default_and_none_is_explicit_unbounded(
    make_registry: RegistryFactory,
    make_policy: PolicyFactory,
    make_store: StoreFactory,
) -> None:
    ...
    store = make_store(registry)
    _seed_library(store, hidden_shelves=0)
    _insert_visible_widgets(store, count=DEFAULT_READ_LIMIT + 1)
    gq = GuardedQuery(store, registry, _pagination_policy(make_policy))

    bounded = gq.get_objects(_narrow(), "Widget")
    assert isinstance(bounded, Page)
    assert len(bounded.items) == DEFAULT_READ_LIMIT
    assert bounded.next_cursor is not None

    unbounded = gq.get_objects(_narrow(), "Widget", limit=None)
    assert isinstance(unbounded, list)
    assert len(unbounded) == DEFAULT_READ_LIMIT + 1


def test_order_by_is_stable_across_page_boundaries_and_uses_row_id_tiebreak(
    make_registry: RegistryFactory,
    make_policy: PolicyFactory,
    make_store: StoreFactory,
) -> None:
    ...
    page1 = gq.get_objects(_narrow(), "Widget", limit=3, order_by="rank")
    page2 = gq.get_objects(
        _narrow(),
        "Widget",
        limit=3,
    ...
    assert [row.payload["id"] for row in page1.items] == ["w-b", "w-e", "w-a"]
    assert [row.payload["id"] for row in page2.items] == ["w-c", "w-d"]
    assert page2.next_cursor is None

    descending = gq.get_objects(
        _narrow(), "Widget", limit=None, order_by=("rank", "desc")
    )
    assert [row.payload["id"] for row in descending] == [
        "w-d",
        "w-a",
        "w-c",
        "w-b",
...
def test_none_order_values_are_deterministic_across_page_boundaries(
    make_registry: RegistryFactory,
    make_policy: PolicyFactory,
    make_store: StoreFactory,
) -> None:
    ...
    asc1 = gq.get_objects(_narrow(), "Widget", limit=2, order_by="rank")
    asc2 = gq.get_objects(
        _narrow(),
        "Widget",
        limit=2,
        order_by=("rank", "asc"),
        after=asc1.next_cursor,
    )
    asc3 = gq.get_objects(
        _narrow(),
        "Widget",
        limit=2,
        order_by=("rank", "asc"),
        after=asc2.next_cursor,
    )

    assert [row.payload["id"] for row in asc1.items] == ["w-none-a", "w-none-b"]
    assert [row.payload["id"] for row in asc2.items] == ["w-none-c", "w-one"]
    assert [row.payload["id"] for row in asc3.items] == ["w-two"]
    assert asc3.next_cursor is None

    desc1 = gq.get_objects(
        _narrow(), "Widget", limit=2, order_by=("rank", "desc")
    )
    desc2 = gq.get_objects(
        _narrow(),
        "Widget",
        limit=2,
        order_by=("rank", "desc"),
        after=desc1.next_cursor,
    )
    desc3 = gq.get_objects(
        _narrow(),
        "Widget",
        limit=2,
        order_by=("rank", "desc"),
        after=desc2.next_cursor,
    )

    assert [row.payload["id"] for row in desc1.items] == ["w-two", "w-one"]
    assert [row.payload["id"] for row in desc2.items] == ["w-none-a", "w-none-b"]
    assert [row.payload["id"] for row in desc3.items] == ["w-none-c"]
    assert desc3.next_cursor is None


@pytest.mark.parametrize(
    ("updated_id", "new_rank", "expected_ids"),
    [
        (
            "w5",
            5,
    ...
    ids=["unread_row_moves_ahead_and_is_dropped", "read_row_moves_behind_and_is_duplicated"],
)
def test_ordered_walk_discloses_non_cursor_update_drop_and_duplicate(
    make_registry: RegistryFactory,
    make_policy: PolicyFactory,
    make_store: StoreFactory,
    updated_id: str,
    ...
    """Pin both residual directions in the amended ordered-walk contract.

    The earlier ordered fixtures were quiescent between pages, while the
    STALE_CURSOR fixtures changed only the page's own cursor row. They could
    not construct a mid-walk write to a different unread or already-read row.
    """
    registry = _pagination_registry(make_registry)
    store = make_store(registry)
    _seed_library(store, hidden_shelves=0)
    ...
    while True:
        ...
        walked.extend(row.payload["id"] for row in page.items)
        if page.next_cursor is None:
            break
        if not updated:
            store.update("Widget", updated_id, {"rank": new_rank}, SRC)
            updated = True
        cursor = page.next_cursor
...
def test_invalid_order_by_shape_or_direction_is_coded(
    make_registry: RegistryFactory,
    make_policy: PolicyFactory,
    make_store: StoreFactory,
    bad_order_by: Any,
    ...
    with raises_code(ValidationFailed, "INVALID_PARAMS"):
        gq.get_objects(_narrow(), "Widget", limit=None, order_by=bad_order_by)
...
def test_ordered_cursor_for_retired_row_raises_stale_cursor(
    make_registry: RegistryFactory,
    make_policy: PolicyFactory,
    make_store: StoreFactory,
) -> None:
    ...
    for widget_id, rank in [
        ...
        store.insert(
            "Widget",
            {"id": widget_id, "shelf_id": "shelf-vis", "rank": rank},
            SRC,
        )
    gq = GuardedQuery(store, registry, _pagination_policy(make_policy))

    page1 = gq.get_objects(
        _narrow(), "Widget", limit=2, order_by=("rank", "asc")
    )
    assert [row.payload["id"] for row in page1.items] == ["w-low", "w-cursor"]
    assert page1.next_cursor is not None

    cursor_row_id = page1.items[-1].payload["id"]
    assert cursor_row_id == "w-cursor"
    store.retire_object("Widget", cursor_row_id)
    ...
    assert "restart the ordered walk from the first page" in str(exc_info.value)
    assert "w-cursor" not in str(exc_info.value)
    assert "20" not in str(exc_info.value)


def test_ordered_retired_and_never_issued_cursors_have_distinct_codes(
    make_registry: RegistryFactory,
    make_policy: PolicyFactory,
    make_store: StoreFactory,
) -> None:
    ...
    assert page1.next_cursor is not None
    store.retire_object("Widget", page1.items[-1].payload["id"])

    with pytest.raises(ValidationFailed) as stale_info:
        gq.get_objects(
            _narrow(),
            "Widget",
            limit=2,
    ...
    with pytest.raises(ValidationFailed) as invalid_info:
        gq.get_objects(
            _narrow(),
            "Widget",
            limit=2,
            order_by=("rank", "asc"),
            after="never-issued-token",
        )

    assert stale_info.value.code == "STALE_CURSOR"
    assert invalid_info.value.code == "INVALID_CURSOR"
    assert stale_info.value.code != invalid_info.value.code
...
def test_after_without_limit_raises_coded_error_on_guarded_query(
    ...
    with raises_code(ValidationFailed, "AFTER_WITHOUT_LIMIT"):
        gq.get_objects(_narrow(), "Widget", limit=None, after="some-cursor")
...
def test_after_without_limit_raises_on_client_list_string_and_typed() -> None:
    ...
    with raises_code(ValidationFailed, "AFTER_WITHOUT_LIMIT"):
        client.list("Widget", limit=None, after="some-cursor")
...
def test_client_list_limit_none_unchanged() -> None:
    ...
    typed = client.list(Widget, limit=None)
    assert isinstance(typed, list)
    assert len(typed) == 12

    stringly = client.list("Widget", limit=None)
