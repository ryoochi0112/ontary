...
- #13 `resolve_consumer` raises -> generic `INTERNAL_ERROR`, raised message
  absent from the payload, over all ten tools.
...
# All ten tools `_register_tools` registers, each paired with arguments that
...
# hardcoded sample -- an eleventh tool must not be able to escape this
# file's assertions).
...
def test_all_tools_matches_what_server_actually_registered() -> None:
    ...
    of bug) -- an eleventh tool must show up here as a failure."""
