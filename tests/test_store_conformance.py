...
from ontary.store._shared import json_references_object
...
def _raw_erasure_rows(store: Store) -> dict[str, list[dict[str, Any]]]:
    """Inspect the three content-bearing tables without using Store reads."""
    if isinstance(store, ObjectStore):
        return {
            "objects": [
                dict(row)
                for row in store._conn.execute(
                    "SELECT * FROM objects WHERE tenant = ? ORDER BY row_id ASC",
                    (store._tenant,),
                ).fetchall()
            ],
            "audit": [
            ...
            "outbox": [
                ...
                for row in store._conn.execute(
                    ...
                    WHERE tenant = ? ORDER BY emitted_at ASC, seq ASC
                    """,
                    (store._tenant,),
                ).fetchall()
            ],
    ...
    if isinstance(store, InMemoryStore):
        return {
            "objects": [dict(row) for row in store._objects],
            "audit": [
                {"seq": seq, **entry.model_dump(mode="json")}
                for seq, entry in enumerate(store._audit, start=1)
            ],
            "outbox": [row.model_dump(mode="json") for row in store._outbox],
        }
    ...
    tables = {
        "objects": "row_id ASC",
        "audit_log": "seq ASC",
        "effect_outbox": "emitted_at ASC, seq ASC",
    }
    result: dict[str, list[dict[str, Any]]] = {}
    with store._conn.cursor() as cursor:
        for table, ordering in tables.items():
            cursor.execute(
                f"SELECT * FROM {table} WHERE tenant = %s ORDER BY {ordering}",
                (store._tenant,),
            )
            columns = [column.name for column in cursor.description]
            key = "audit" if table == "audit_log" else table.removeprefix("effect_")
            result[key] = [
...
def _without_content(
    rows: list[dict[str, Any]], *content_fields: str
) -> list[dict[str, Any]]:
    return [
        {key: value for key, value in row.items() if key not in content_fields}
        for row in rows
    ]
...
ERASURE_REFERENCE_SHAPES: tuple[tuple[str, dict[str, Any]], ...] = (
    (
        "params-id-key",
        {
            "audit": {
                "target_type": "Company",
                "target_id": "company-r4-other",
                "params": {
    ...
    (
        "params-free-text",
        {
            "audit": {
                ...
                "params": {
                    "message": (
                        "A prose mention names Department {object_id}; "
                        "the content marker is {marker}."
                    )
                },
            }
        },
                ...
                "effects": [
                    {
                        ...
                        "outcome": "failed",
                        "error": None,
                        "effect_id": "effect-r4-effects-payload",
                    }
                ],
            }
        },
    ...
    (
        "effects-error",
        {
            "audit": {
                ...
                "effects": [
                    {
                        "api_name": "r4-effects-error",
                        "payload": {},
                        "outcome": "failed",
                        "error": "{marker}",
                        "effect_id": "effect-r4-effects-error",
                    }
                ],
            }
        },
    ...
    (
        "outbox-payload",
        {
            "audit": {
                "target_type": "Department",
                "target_id": "{object_id}",
                "invocation_id": "inv-r4-outbox-payload",
            },
            "outbox": {
                "invocation_id": "inv-r4-outbox-payload",
    ...
    (
        "outbox-last-error",
        {
            "audit": {
                "target_type": "Department",
                "target_id": "{object_id}",
                "invocation_id": "inv-r4-outbox-last-error",
            },
            "outbox": {
                "invocation_id": "inv-r4-outbox-last-error",
                "payload": {},
    ...
    (
        "writes-object",
        {
            "audit": {
                ...
                "invocation_id": "inv-r4-writes-object",
            },
            "outbox": {
                "invocation_id": "inv-r4-writes-object",
        ...
        "writes-link",
        {
            "audit": {
                ...
                "writes": [
                    {
                        "op": "unlink",
                        "link_type": "belongsToDepartment",
                        "from_id": "team-r4-cascade",
                        "to_id": "{object_id}",
                    }
                ],
                "invocation_id": "inv-r4-writes-link",
            },
            "outbox": {
                "invocation_id": "inv-r4-writes-link",
                "payload": {"message": "{marker}"},
                ...
                "writes": [
                    {
                        "op": "update",
                        "object_type": "Company",
                        "object_id": "company-r4-another",
                    },
                    {
                        "op": "unlink",
                        "link_type": "belongsToDepartment",
                        "from_id": "team-r4-cross-target",
                        "to_id": "{object_id}",
                    },
                ],
                "invocation_id": "inv-r4-cross-target",
            },
            "outbox": {
                "invocation_id": "inv-r4-cross-target",
                "payload": {"message": "{marker}"},
    ...
    (
        ...
        {
            "outbox": {
                "invocation_id": "inv-r4-no-purged-audit",
                "payload": {
                    "object_type": "Department",
                    "object_id": "{object_id}",
                    "message": "{marker}",
...
def _render_erasure_shape_template(value: Any, *, object_id: str, marker: str) -> Any:
    if isinstance(value, str):
        return value.format(object_id=object_id, marker=marker)
    if isinstance(value, list):
        return [
            _render_erasure_shape_template(item, object_id=object_id, marker=marker)
            for item in value
        ]
    if isinstance(value, dict):
        return {
            key: _render_erasure_shape_template(item, object_id=object_id, marker=marker)
            for key, item in value.items()
        }
    return value


def _seed_erasure_reference_shape(
    store: Store,
    shape_name: str,
    ...
    *,
    ...
    rendered = _render_erasure_shape_template(shape_template, object_id=object_id, marker=marker)
    assert isinstance(rendered, dict)
    audit_data = rendered.get("audit")
    audit_time = datetime(2026, 8, 25, 6, 7, 8, tzinfo=timezone.utc)
    if audit_data is not None:
        ...
        store.append_audit(
            AuditEntry(
                ...
                actor="operator",
                role="DataSteward",
                action=f"R4_{shape_name}",
                target_type=audit_data["target_type"],
                target_id=audit_data["target_id"],
                params=audit_data.get("params", {}),
                outcome="ok",
                writes=[WriteRecord(**write) for write in audit_data.get("writes", [])],
                effects=[EffectRecord(**effect) for effect in audit_data.get("effects", [])],
                invocation_id=audit_data.get("invocation_id"),
            )
        )

    outbox_data = rendered.get("outbox")
    if outbox_data is None:
        return
    assert isinstance(outbox_data, dict)
    store.enqueue_effects(
        [
            OutboxRecord(
                effect_id=f"effect-r4-{shape_name}",
                invocation_id=outbox_data["invocation_id"],
                seq=0,
                api_name=f"r4-{shape_name}",
                payload=outbox_data.get("payload", {}),
                action=f"R4_{shape_name}",
                actor_id="operator",
                role="DataSteward",
                emitted_at=audit_time,
                state="failed",
                attempts=1,
                next_attempt_at=audit_time,
                last_error=outbox_data.get("last_error"),
...
@pytest.mark.parametrize(
    "reference",
    [
        {"object_id": "ticket-erase"},
        {"target_id": "ticket-erase"},
        {"ticket_ids": ["ticket-keep", "ticket-erase"]},
    ],
    ids=("object-id", "target-id", "plural-typed-ids"),
)
def test_json_reference_generic_ids_without_paired_type_match_like_bare_id(
    reference: dict[str, Any],
) -> None:
    assert json_references_object({"id": "ticket-erase"}, "Ticket", "ticket-erase")
    assert json_references_object(reference, "Ticket", "ticket-erase")


@pytest.mark.parametrize(
    "reference",
    [
        {"object_type": "TEAM", "id": "team-erase"},
        {"target_type": "team", "id": "team-erase"},
    ],
    ids=("object-type", "target-type"),
)
def test_json_reference_bare_id_with_casefold_equal_stated_type_matches(
    reference: dict[str, Any],
) -> None:
    assert json_references_object(reference, "Team", "team-erase")


def test_json_reference_bare_id_with_different_stated_type_is_suppressed() -> None:
    assert not json_references_object(
        {"object_type": "Company", "id": "team-erase"},
        "Team",
        "team-erase",
    )


def test_json_reference_generic_id_with_different_paired_type_is_suppressed() -> None:
    assert not json_references_object(
        {"object_type": "Other", "object_id": "ticket-erase"},
        "Ticket",
        "ticket-erase",
    )


def test_json_reference_generic_id_with_casefold_equal_paired_type_matches() -> None:
    assert json_references_object(
        {"object_type": "ticket", "object_id": "ticket-erase"},
        "Ticket",
        "ticket-erase",
    )
...
@pytest.mark.parametrize(
    "paired_type",
    [None, 7, [], {}],
    ids=("null", "number", "list", "dict"),
)
def test_json_reference_generic_id_with_non_string_paired_type_matches(
    paired_type: Any,
) -> None:
    assert json_references_object(
        {"object_type": paired_type, "object_id": "ticket-erase"},
        "Ticket",
        "ticket-erase",
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {
            "object_type": "Company",
            "owner_ticket_id": "ticket-erase",
        },
    ],
    ids=("plural-list-in-differently-typed-dict", "foreign-key-in-differently-typed-dict"),
)
def test_json_reference_typed_keys_ignore_unpaired_container_type(
    reference: dict[str, Any],
) -> None:
    assert json_references_object(reference, "Ticket", "ticket-erase")


def test_json_reference_typed_key_with_different_paired_type_is_suppressed() -> None:
    assert not json_references_object(
        {
            "owner_ticket_type": "Other",
            "owner_ticket_id": "ticket-erase",
        },
        "Ticket",
        "ticket-erase",
...
@pytest.mark.parametrize(
    ("shape_name", "shape_template"),
    ERASURE_REFERENCE_SHAPES,
    ids=[shape_name for shape_name, _ in ERASURE_REFERENCE_SHAPES],
)
def test_erase_object_content_destroys_every_reference_shape(
    store: Store, shape_name: str, shape_template: dict[str, Any]
) -> None:
    """Every seeded marker disappears from the backend's raw row sweep.

    The template carries both the reference that makes a row eligible and the
    content marker to erase. Prose in params, effect errors, and outbox
    last_error are content locations rather than discovery paths, so those
    cases use the row's direct target or invocation join as their reference.
    """
    object_id = f"department-r4-{shape_name}"
    marker = f"r4-erasure-marker-{shape_name}"
    store.insert(
        "Department",
        {"id": object_id, "name": marker},
        Source(source_system="r4-invariant"),
    )
    _seed_erasure_reference_shape(
        store, shape_name, shape_template, object_id=object_id, marker=marker
    )

    before = _raw_erasure_rows(store)
    before_seeded = {k: v for k, v in before.items() if k != "objects"}
    assert marker in json.dumps(before_seeded, default=str)

    result = store.erase_object_content("Department", object_id)
    after = _raw_erasure_rows(store)

    assert result.object_rows_purged == 1
    assert result.audit_entries_purged + result.outbox_rows_purged >= 1
    assert marker not in json.dumps(after, default=str)


def test_object_erasure_state_reports_live_and_closed_content_states(
    store: Store,
) -> None:
    """The probe's content flag distinguishes live, closed, and purged rows."""
    live_id = store.insert(
        "Team",
        {"id": "team-state-live", "name": "live content"},
        Source(source_system="state-pin"),
    )
    closed_id = store.insert(
        "Team",
        {"id": "team-state-closed", "name": "closed content"},
        Source(source_system="state-pin"),
    )
    store.retire_object("Team", closed_id)

    purged_id = store.insert(
        "Team",
        {"id": "team-state-purged", "name": "purged content"},
        Source(source_system="state-pin"),
    )
    store.retire_object("Team", purged_id)
    assert store.erase_object_content("Team", purged_id).object_rows_purged == 1

    assert store.object_erasure_state("Team", live_id) == (True, True)
    assert store.object_erasure_state("Team", closed_id) == (True, True)
    assert store.object_erasure_state("Team", purged_id) == (True, False)


def test_erase_object_content_tombstones_all_content_and_preserves_structure(
    store: Store,
) -> None:
    """Erasure is the sole deliberate exception to audit content immutability.

    It may replace content fields with empty JSON objects, but it must not
    delete/reorder rows or alter any audit field other than ``params`` and
    ``effects``. The purged and kept rows deliberately carry distinctive
    non-default ``writes``, ``capability_accesses``, and ``principal`` values,
    so the unchanged comparison below pins that claim across the high-risk
    erasure seam.
    """
    object_id = store.insert(
        "Team",
        {"id": "team-erase", "name": "target-secret-history"},
        Source(source_system="connector", source_id="source-history"),
    )
    store.update(
        "Team",
        object_id,
        {"name": "target-secret-current"},
        Source(source_system="connector", source_id="source-current"),
    )
    store.insert(
        "Team",
        {"id": "team-keep", "name": "unrelated-content"},
        Source(source_system="connector", source_id="source-keep"),
    )

    audit_time = datetime(2026, 8, 25, 1, 2, 3, tzinfo=timezone.utc)
    store.append_audit(
        AuditEntry(
            ts=audit_time,
            actor="operator",
            ...
            target_type="Team",
            target_id=object_id,
            params={"name": "target-secret-audit-target"},
            outcome="ok",
            principal="principal-r4-purged",
            writes=[
                WriteRecord(
                    op="update",
                    object_type="Team",
            ...
            capability_accesses=[CapabilityAccessRecord(api_name="r4-purged-capability", count=7)],
            effects=[
                EffectRecord(
                    api_name="notify-primary",
                    payload={"message": "target-secret-effect-payload-direct"},
                    outcome="dispatched",
                    error="target-secret-effect-error-direct",
                    effect_id="effect-audit-direct-1",
                ),
                EffectRecord(
                    api_name="notify-secondary",
                    payload={"message": "target-secret-effect-payload-ordered"},
                    outcome="failed",
                    error="target-secret-effect-error-ordered",
                    effect_id="effect-audit-direct-2",
                ),
            ],
    ...
    store.append_audit(
        AuditEntry(
            ts=audit_time + timedelta(seconds=1),
            actor="operator",
            role="DataSteward",
            action="ReferenceTeam",
            target_type="Company",
            target_id="company-1",
            params={"name": "reference exists only in effects"},
            outcome="ok",
            effects=[
                EffectRecord(
                    api_name="sync-team",
                    payload={
                        ...
                        "message": "target-secret-effect-payload-only-reference",
                    },
                    outcome="retrying",
                    error="target-secret-effect-error-only-reference",
                    effect_id="effect-audit-reference",
                )
            ],
            invocation_id="inv-reference",
        )
    )
    store.append_audit(
        AuditEntry(
            ts=audit_time + timedelta(seconds=2),
            actor="operator",
            role="DataSteward",
            action="KeepCompany",
            target_type="Company",
            target_id="company-keep",
            params={"name": "unrelated-audit-content"},
            outcome="denied",
            principal="principal-r4-kept",
            writes=[
                WriteRecord(
                    op="update",
                    object_type="Company",
                    object_id="company-keep",
                )
            ],
            capability_accesses=[CapabilityAccessRecord(api_name="r4-kept-capability", count=11)],
            effects=[
                EffectRecord(
                    api_name="keep-company",
                    payload={"message": "unrelated-effect-content"},
                    outcome="failed",
                    error="unrelated-effect-error",
                    effect_id="effect-audit-keep",
                )
            ],
    ...
    def outbox_record(
        ...
        emitted_at = audit_time + timedelta(seconds=offset)
        return OutboxRecord(
            effect_id=effect_id,
    ...
    store.enqueue_effects(
        [
            outbox_record(
                "effect-delivered",
                "inv-target",
                {"message": "target-secret-delivered"},
                "delivered",
                3,
                "target-secret-outbox-last-error-direct",
            ),
            outbox_record(
                "effect-failed",
                "inv-reference",
                {"message": "target-secret-failed"},
                "failed",
                4,
                "target-secret-outbox-last-error-reference",
            ),
            outbox_record(
                "effect-keep",
                "inv-keep",
                {"message": "unrelated-outbox-content"},
                "delivered",
                5,
                "unrelated-outbox-error",
            ),
        ]
    )

    before = _raw_erasure_rows(store)
    result = store.erase_object_content("Team", object_id)
    after = _raw_erasure_rows(store)

    assert result.object_rows_purged == 2
    assert result.audit_entries_purged == 2
    assert result.outbox_rows_purged == 2
    assert {table: len(rows) for table, rows in after.items()} == {
        table: len(rows) for table, rows in before.items()
    }

    before_objects = _without_content(before["objects"], "payload")
    after_objects = _without_content(after["objects"], "payload")
    assert [
        {key: value for key, value in row.items() if key != "valid_to"}
        for row in after_objects
    ] == [
        {key: value for key, value in row.items() if key != "valid_to"}
    ...
    target_rows = [
        ...
        if row["object_type"] == "Team" and str(row["id"]) == object_id
    ]
    assert len(target_rows) == 2
    assert all(_decoded_content(row["payload"]) == {} for row in target_rows)
    assert all(row["valid_to"] is not None for row in target_rows)
    kept_object = next(
        row for row in after["objects"] if str(row["id"]) == "team-keep"
    )
    assert _decoded_content(kept_object["payload"])["name"] == "unrelated-content"

    assert _without_content(after["audit"], "params", "effects") == _without_content(
        before["audit"], "params", "effects"
    )
    assert [_decoded_content(row["params"]) for row in after["audit"]] == [
        {},
        {},
        {"name": "unrelated-audit-content"},
    ]
    assert "target-secret" not in json.dumps(after, default=str)
    assert [_decoded_content(row["effects"]) for row in after["audit"]] == [
        [
            {
                "api_name": "notify-primary",
                "payload": {},
    ...
    assert _without_content(
        after["outbox"], "payload", "last_error"
    ) == _without_content(before["outbox"], "payload", "last_error")
    assert [_decoded_content(row["payload"]) for row in after["outbox"]] == [
        {},
        {},
        {"message": "unrelated-outbox-content"},
    ]
    assert [row["last_error"] for row in after["outbox"]] == [
        None,
        None,
        "unrelated-outbox-error",
    ]

    second = store.erase_object_content("Team", object_id)
    assert second.object_rows_purged == 0
    assert second.audit_entries_purged == 0
    assert second.outbox_rows_purged == 0
    assert _raw_erasure_rows(store) == after


def test_erase_object_content_follows_cascade_writes_to_audit_and_outbox(
    store: Store,
) -> None:
    """A cascade unlink is an erasure reference even when the target differs."""
    department_id = store.insert(
        "Department",
        {"id": "department-erase", "name": "department secret"},
        Source(source_system="connector"),
    )
    store.insert(
        "Team",
        {"id": "team-1", "name": "team secret"},
        Source(source_system="connector"),
    )
    audit_time = datetime(2026, 8, 25, 2, 3, 4, tzinfo=timezone.utc)
    invocation_id = "inv-cascade-writes"
    store.append_audit(
        AuditEntry(
            ts=audit_time,
            actor="operator",
            ...
            effects=[
                EffectRecord(
                    api_name="notify-team",
                    payload={"message": "team retirement secret"},
                    outcome="failed",
                    error="team retirement effect error",
                    effect_id="effect-cascade-writes",
                )
            ],
            invocation_id=invocation_id,
        )
    ...
    store.enqueue_effects(
        [
            OutboxRecord(
                ...
                seq=0,
                api_name="notify-team",
                payload={"message": "team retirement outbox secret"},
                action="RetireTeam",
                actor_id="operator",
                role="DataSteward",
                emitted_at=audit_time,
                ...
                attempts=1,
                next_attempt_at=audit_time,
                last_error="team retirement outbox error",
                updated_at=audit_time,
            )
        ]
    )
    ...
    result = store.erase_object_content("Department", department_id)
    after = _raw_erasure_rows(store)

    assert (
        result.object_rows_purged,
        result.audit_entries_purged,
        result.outbox_rows_purged,
    ) == (1, 1, 1)
    assert _decoded_content(after["audit"][0]["params"]) == {}
    assert _decoded_content(after["audit"][0]["effects"]) == [
        {
            "api_name": "notify-team",
            "payload": {},
            "outcome": "failed",
    ...
    assert after["audit"][0]["writes"] == before["audit"][0]["writes"]
    assert _decoded_content(after["outbox"][0]["payload"]) == {}
    assert after["outbox"][0]["last_error"] is None

    second = store.erase_object_content("Department", department_id)
    assert (
        second.object_rows_purged,
        second.audit_entries_purged,
        second.outbox_rows_purged,
    ) == (0, 0, 0)
    assert _raw_erasure_rows(store) == after


def test_erase_object_content_follows_cross_target_object_writes(
    store: Store,
) -> None:
    department_id = store.insert(
        "Department",
        {"id": "department-object-write", "name": "department secret"},
        Source(source_system="connector"),
    )
    store.append_audit(
        AuditEntry(
            actor="operator",
            role="DataSteward",
            action="UpdateDepartmentForTeam",
            target_type="Team",
            target_id="team-1",
            params={"team_id": "team-1"},
            outcome="ok",
    ...
    assert result.audit_entries_purged == 1
    assert _decoded_content(after["audit"][0]["params"]) == {}
    assert after["audit"][0]["writes"] == before["audit"][0]["writes"]


def test_erase_object_content_does_not_follow_differently_typed_link_endpoint(
    store: Store,
) -> None:
    """A same-valued Team endpoint is not a reference to a Department."""
    shared_id = "shared-9"
    store.insert(
        "Department",
        {"id": shared_id, "name": "department subject"},
        Source(source_system="connector"),
    )
    store.insert(
        "Team",
        {"id": shared_id, "name": "team subject"},
        Source(source_system="connector"),
    )
    audit_time = datetime(2026, 8, 25, 3, 4, 5, tzinfo=timezone.utc)
    invocation_id = "inv-team-shared-id"
    store.append_audit(
        AuditEntry(
            ts=audit_time,
            actor="operator",
            ...
            target_type="Team",
            target_id=shared_id,
            params={"message": "team audit params evidence"},
            outcome="ok",
            writes=[
                WriteRecord(
                    op="unlink",
                    link_type="belongsToDepartment",
                    from_id=shared_id,
                    to_id="other-department",
                )
            ],
            effects=[
                EffectRecord(
                    api_name="notify-team",
                    payload={"message": "team audit effect evidence"},
                    outcome="failed",
                    error="team audit error evidence",
                    effect_id="effect-team-shared-id",
                )
            ],
            invocation_id=invocation_id,
        )
    ...
    store.enqueue_effects(
        [
            OutboxRecord(
                ...
                seq=0,
                api_name="notify-team",
                payload={"message": "team outbox payload evidence"},
                action="MoveTeam",
                actor_id="operator",
                role="DataSteward",
                emitted_at=audit_time,
                ...
                attempts=1,
                next_attempt_at=audit_time,
                last_error="team outbox error evidence",
                updated_at=audit_time,
            )
        ]
    )
    ...
    result = store.erase_object_content("Department", shared_id)
    after = _raw_erasure_rows(store)
...
def test_erase_object_content_does_not_follow_endpoint_owned_by_a_retired_object(
    store: Store,
) -> None:
    """"An object of that type exists" means EVER existed, retired included.

    The live-decoy sibling above always inserts a LIVE Team, so it cannot
    tell "a Team row carries this id" from "a LIVE Team row carries this id"
    -- and the second reading is the R1 collateral-deletion defect one step
    sideways: retire the Team first and erasing the same-id Department
    destroys the Team's audit params, effect payload and outbox content,
    which no erasure request ever asked for. Retention outlives retirement;
    an erasure is scoped to its subject, not to whatever currently happens
    to be live.
    """
    shared_id = "shared-retired-9"
    store.insert(
        "Department",
        {"id": shared_id, "name": "department subject"},
        Source(source_system="connector"),
    ...
    audit_time = datetime(2026, 8, 28, 6, 7, 8, tzinfo=timezone.utc)
    invocation_id = "inv-team-retired-shared-id"
    store.append_audit(
        AuditEntry(
            ts=audit_time,
            actor="operator",
            ...
            target_type="Team",
            target_id=shared_id,
            params={"message": "retired team audit params evidence"},
            outcome="ok",
            writes=[
                WriteRecord(
                    op="unlink",
            ...
            effects=[
                EffectRecord(
                    api_name="notify-team",
                    payload={"message": "retired team audit effect evidence"},
                    outcome="failed",
                    error="retired team audit error evidence",
                    effect_id="effect-team-retired-shared-id",
                )
            ],
            invocation_id=invocation_id,
        )
    ...
    store.enqueue_effects(
        [
            OutboxRecord(
                ...
                seq=0,
                api_name="notify-team",
                payload={"message": "retired team outbox payload evidence"},
                action="MoveTeam",
                actor_id="operator",
                role="DataSteward",
                emitted_at=audit_time,
                ...
                attempts=1,
                next_attempt_at=audit_time,
                last_error="retired team outbox error evidence",
                updated_at=audit_time,
            )
        ]
    )
    # The one thing the sibling never does.
    store.retire_object("Team", shared_id)
    before = _raw_erasure_rows(store)
...
def test_erase_object_content_follows_link_endpoint_no_declared_object_can_own(
    store: Store,
) -> None:
    """The declared endpoint type excuses an id only when that object exists.

    `create_link` validates neither endpoint existence nor endpoint TYPE, so
    `belongsToDepartment`'s `from_type="Team"` is not evidence that a
    `from_id` names a Team. With no Team row carrying this id, the only
    object the endpoint can name is the Department being erased -- so the
    audit and outbox evidence it produced must be purged.

    The sibling above is the other half of the same rule: there a Team row
    DOES carry the id, and the same write is correctly left alone. Its
    fixture cannot construct this case -- it always inserts the Team.
    """
    department_id = "department-9"
    store.insert(
        "Department",
        {"id": department_id, "name": "department subject"},
        Source(source_system="connector"),
    )
    audit_time = datetime(2026, 8, 28, 3, 4, 5, tzinfo=timezone.utc)
    invocation_id = "inv-endpoint-unowned"
    store.append_audit(
        AuditEntry(
            ts=audit_time,
            actor="operator",
            ...
            target_type="Team",
            target_id="team-7",
            params={"message": "audit params evidence"},
            outcome="ok",
            writes=[
                WriteRecord(
                    op="unlink",
            ...
            effects=[
                EffectRecord(
                    api_name="notify-team",
                    payload={"message": "audit effect evidence"},
                    outcome="failed",
                    error="audit error evidence",
                    effect_id="effect-endpoint-unowned",
                )
            ],
            invocation_id=invocation_id,
        )
    ...
    store.enqueue_effects(
        [
            OutboxRecord(
                ...
                seq=0,
                api_name="notify-team",
                payload={"message": "outbox payload evidence"},
                action="MoveTeam",
                actor_id="operator",
                role="DataSteward",
                emitted_at=audit_time,
                ...
                attempts=1,
                next_attempt_at=audit_time,
                last_error="outbox error evidence",
                updated_at=audit_time,
            )
        ]
    )
    ...
    assert entry.params == {}
    assert entry.effects[0].payload == {}
    assert entry.effects[0].error is None
    outbox_row = store.outbox_entries()[0]
    assert outbox_row.payload == {}
    assert outbox_row.last_error is None
    # Content destroyed, structure intact: the write that PROVED the
    # reference is still there to be audited afterwards.
    assert entry.writes == [
        WriteRecord(
            op="unlink",
            link_type="belongsToDepartment",
...
def test_erase_object_content_follows_removed_link_type_untyped(
    store: Store,
) -> None:
    """Historical writes remain detectable after their link type is removed."""
    department_id = store.insert(
        "Department",
        {"id": "department-removed-link", "name": "department secret"},
        Source(source_system="connector"),
    )
    audit_time = datetime(2026, 8, 25, 4, 5, 6, tzinfo=timezone.utc)
    invocation_id = "inv-removed-link"
    store.append_audit(
        AuditEntry(
            ts=audit_time,
            actor="operator",
            role="DataSteward",
            action="HistoricalUnlink",
            target_type="Team",
            target_id="team-historical",
            params={"message": "historical audit evidence"},
            outcome="ok",
            writes=[
                WriteRecord(
                    op="unlink",
                    link_type="removedBelongsToDepartment",
                    from_id="team-historical",
                    to_id=department_id,
                )
            ],
            effects=[
                EffectRecord(
                    api_name="notify-history",
                    payload={"message": "historical effect evidence"},
                    outcome="failed",
                    error="historical effect error",
                    effect_id="effect-removed-link",
                )
            ],
            invocation_id=invocation_id,
        )
    ...
    store.enqueue_effects(
        [
            OutboxRecord(
                ...
                seq=0,
                api_name="notify-history",
                payload={"message": "historical outbox evidence"},
                action="HistoricalUnlink",
                actor_id="operator",
                role="DataSteward",
                emitted_at=audit_time,
                ...
                attempts=1,
                next_attempt_at=audit_time,
                last_error="historical outbox error",
                updated_at=audit_time,
            )
        ]
    )
    ...
    assert _decoded_content(after["audit"][0]["params"]) == {}
    assert _decoded_content(after["audit"][0]["effects"])[0]["payload"] == {}
    assert after["audit"][0]["writes"] == before["audit"][0]["writes"]
    assert _decoded_content(after["outbox"][0]["payload"]) == {}
    assert after["outbox"][0]["last_error"] is None
        ...
        (
            {
                "object_type": "Team",
                "team_ids": ["team-erase"],
                "names": ["target-secret-audit-id-list"],
            },
            "target-secret-audit-id-list",
        ),
        (
            {
                "object_type": "Company",
                "id": "company-owner",
                "owner_team_id": "team-erase",
                "team_name": "target-secret-audit-foreign-key",
            },
            "target-secret-audit-foreign-key",
        ),
    ],
    ids=("plural-id-list", "foreign-key-with-containing-type"),
)
def test_erase_object_content_redacts_explicit_audit_param_reference_shapes(
    store: Store, params: dict[str, Any], secret: str
) -> None:
    object_id = store.insert(
        "Team",
        {"id": "team-erase", "name": "target object"},
        Source(source_system="connector"),
    )
    store.append_audit(
        AuditEntry(
    ...
    assert _without_content(after["audit"], "params") == _without_content(
        before["audit"], "params"
    )
    assert _decoded_content(after["audit"][0]["params"]) == {}
    assert secret not in json.dumps(after, default=str)


def test_erase_object_content_preserves_colliding_nonreference_id_params(
    store: Store,
) -> None:
    object_id = store.insert(
        "Team",
        {"id": "shared-id", "name": "target object"},
        Source(source_system="connector"),
    )
    store.append_audit(
        AuditEntry(
            ...
            params={
                "object_type": "Company",
                "id": "company-owner",
                "company_id": "shared-id",
                "invocation_id": "shared-id",
                "name": "unrelated-company-secret",
            },
            outcome="ok",
        )
    )
    ...
    assert result.audit_entries_purged == 0
    assert after["audit"] == before["audit"]


def test_erase_object_content_rolls_back_every_tombstone_on_failure(
    store: Store, monkeypatch: pytest.MonkeyPatch
) -> None:
    object_id = store.insert(
        "Team",
        {"id": "team-atomic", "name": "atomic-secret"},
        Source(source_system="connector"),
    )
    store.append_audit(
        AuditEntry(
            ...
            target_type="Team",
            target_id=object_id,
            params={"message": "atomic-secret-audit"},
            outcome="ok",
            effects=[
                EffectRecord(
                    api_name="notify",
                    payload={"message": "atomic-secret-effect"},
                    outcome="failed",
                    error="atomic-secret-effect-error",
                    effect_id="effect-atomic-audit",
                )
            ],
            invocation_id="inv-atomic",
        )
    )
    outbox_time = datetime(2026, 8, 25, tzinfo=timezone.utc)
    store.enqueue_effects(
        [
            OutboxRecord(
                effect_id="effect-atomic-first",
                invocation_id="inv-atomic",
                seq=0,
                api_name="notify",
                payload={"message": "atomic-secret-outbox"},
                action="Notify",
                actor_id="operator",
                role="DataSteward",
                emitted_at=outbox_time,
                state="failed",
                attempts=1,
                next_attempt_at=outbox_time,
                last_error="atomic-secret-outbox-error",
                updated_at=outbox_time,
            ),
            OutboxRecord(
                effect_id="effect-atomic-trigger",
                invocation_id="inv-trigger",
                seq=0,
                api_name="notify",
                payload={"object_type": "Team", "object_id": object_id},
                action="Notify",
                actor_id="operator",
                role="DataSteward",
                emitted_at=outbox_time + timedelta(seconds=1),
                state="failed",
                attempts=1,
                next_attempt_at=outbox_time,
                last_error="trigger failure after prior tombstones",
                updated_at=outbox_time,
            ),
        ]
    )
    ...
    def fail_reference_check(value: Any, object_type: str, obj_id: str) -> bool:
        raise RuntimeError("injected erasure failure")

    backend_module = sys.modules[type(store).__module__]
    monkeypatch.setattr(backend_module, "json_references_object", fail_reference_check)

    with pytest.raises(RuntimeError, match="injected erasure failure"):
        store.erase_object_content("Team", object_id)

    assert _raw_erasure_rows(store) == before
    assert store.read_current("Team", object_id) is not None
...
def test_audit_log_is_append_only_no_update_delete_api(store: Store) -> None:
    """Erasure stays outside generic audit mutation APIs.

    ``erase_object_content`` is deliberately not added to this banned-name
    set: the erasure conformance pin above proves its narrower exception may
    blank content while audit row count, ordering, and every non-content field
    remain unchanged.
    """
