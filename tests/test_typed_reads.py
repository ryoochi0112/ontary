...
from ontary._typed_api import _TypedReadMixin
...
class TestTypedGetList:
    ...
    def test_list_returns_model_instances(self) -> None:
        ...
        tickets = client.list(Ticket, limit=None)
    ...
    def test_typed_and_string_where_share_operator_json_and_date_semantics(
        self,
    ) -> None:
        client, Ticket, _ = _client(_human_consumer())
        client.ingest(
        ...
        where = {"occurred_on": {"gte": date(2026, 2, 15)}}

        typed = client.list(Ticket, where=where, limit=None)
        dynamic = client.list("Ticket", where=where, limit=None)

        assert [ticket.id for ticket in typed] == ["t2", "t3"]
        assert [row.payload["id"] for row in dynamic] == ["t2", "t3"]
        assert [row.payload["occurred_on"] for row in dynamic] == [
            "2026-02-15",
            "2026-03-15",
        ]
...
def _build_linked_ontology() -> (
    ...
    works_with = ontology.link("worksWith", Ticket, Person, Cardinality.MANY_TO_MANY)
...
class TestTypedTraverse:
    def test_traverse_handle_reverse_flag_is_symmetric(self) -> None:
        client, Ticket, Person, works_with, _, _ = _linked_client(_human_consumer())
        client.ingest(
            "Ticket",
            [
                {"id": "ticket-a", "subject": "a", "priority": 1},
                {"id": "ticket-c", "subject": "c", "priority": 1},
            ],
            SRC,
        )
        client.ingest(
            "Person",
            [{"id": "person-b", "name": "B"}, {"id": "person-d", "name": "D"}],
            SRC,
        )
        client.ingest_links(
            "worksWith",
            [("ticket-a", "person-b"), ("ticket-c", "person-b"), ("ticket-a", "person-d")],
            SRC,
        )

        forward = client.traverse(works_with, "ticket-a")
        reverse = client.traverse(works_with, "person-b", reverse=True)
        assert all(isinstance(row, Person) for row in forward)
        assert all(isinstance(row, Ticket) for row in reverse)
        assert {row.id for row in forward} == {"person-b", "person-d"}
        assert {row.id for row in reverse} == {"ticket-a", "ticket-c"}
    ...
    def test_identity_revealing_link_typed_denied_for_human(self) -> None:
        ...
        with raises_code(VisibilityError, "VISIBILITY_DENIED"):
            client.traverse(assigned_to, "p1", reverse=True)
...
class TestTypedAggregate:
    @pytest.mark.parametrize(
        ("typed", "grouped"),
        [
            pytest.param(False, False, id="string-aggregate"),
            pytest.param(True, False, id="typed-aggregate"),
            pytest.param(False, True, id="string-aggregate-by"),
            pytest.param(True, True, id="typed-aggregate-by"),
        ],
    )
    def test_only_an_engine_minted_dispatch_carries_author_provenance(
        self,
        typed: bool,
        grouped: bool,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Every read surface a CALLER can build reaches `_aggregate` at
        consumer tier; only the engine's per-dispatch clone is granted.

        This assertion is the inversion of what it pinned before, and the
        inversion is the point. It used to read
        `["consumer", "author_function", "consumer", "consumer"]`, where the
        author-tier entry was a hand-constructed
        `BoundQuery(query, consumer, registry)` -- finding 4's second door,
        pinned as intended behaviour. `BoundQuery` is exported, so that made
        the tier public API.

        The fourth surface (`FutureReadSurface`) is why the default lives on
        `_TypedReadMixin` and no subclass restates it: a read façade added
        later inherits the fail-closed tier without its author thinking
        ...
        client.ingest(
            "Ticket",
            [
                {"id": "t1", "subject": "s", "priority": 1},
                {"id": "t2", "subject": "s", "priority": 2},
                {"id": "t3", "subject": "s", "priority": 3},
            ],
            SRC,
        )
        ...
        query = client._query
        grants: list[object] = []
        original_aggregate = query._aggregate

        def capture_grant(*args: Any, **kwargs: Any) -> Any:
            grants.append(kwargs.get("_author_dispatch"))
            return original_aggregate(*args, **kwargs)

        monkeypatch.setattr(query, "_aggregate", capture_grant)

        def invoke(surface: Any) -> None:
            if grouped:
                if typed:
                    surface.aggregate_by(Ticket, "priority", "subject")
                else:
                    surface.aggregate_by("Ticket", "priority", "subject")
            elif typed:
                surface.aggregate(Ticket, "priority")
            else:
                surface.aggregate("Ticket", "priority")

        invoke(client)
        bound = BoundQuery(query, _human_consumer(), client._registry)
        invoke(bound)

        class FutureReadSurface(_TypedReadMixin):
            _typed_owner = "future"

        future = FutureReadSurface()
        future._query = query
        future._consumer = _human_consumer()
        future._registry = client._registry
        invoke(future)

        if grouped:
            query.aggregate_by(_human_consumer(), "Ticket", "priority", "subject")
        else:
            query.aggregate(_human_consumer(), "Ticket", "priority")

        # ...and the one surface a caller cannot build: the throwaway
        # `BoundQuery` the engine mints for a declared-Function dispatch.
        invoke(bound._for_author_dispatch(frozenset()))

        assert grants == [None, None, None, None, _AUTHOR_DISPATCH]
    ...
    def test_aggregate_typed_forwards_func(self) -> None:
        client, Ticket, _, _, _, _ = _linked_client(_human_consumer())
        client.ingest(
            "Ticket",
            [
...
class TestTypedCountExists:
    def test_typed_and_string_forms_apply_operator_where_without_min_n(self) -> None:
        client, Note = _counting_client(on_t1=3, on_t2=4)
        where = {"id": {"contains": "t1-1"}}

        assert client.count(Note, where=where) == 1
        assert client.count("Note", where=where) == 1
        assert client.exists(Note, where=where) is True
        assert client.exists("Note", where=where) is True
        missing = {"id": {"contains": "missing"}}
        assert client.exists(Note, where=missing) is False
        assert client.exists("Note", where=missing) is False

    def test_scoped_away_rows_return_zero_and_false(self) -> None:
        client, Note = _counting_client(on_t1=0, on_t2=4)

        assert client.count(Note) == client.count("Note") == 0
        assert client.exists(Note) is False
        assert client.exists("Note") is False

    @pytest.mark.parametrize("method_name", ["count", "exists"])
    def test_typed_unknown_where_key_keeps_unknown_field(
        self, method_name: str
    ) -> None:
        client, Note = _counting_client(on_t1=1)

        with raises_code(ValidationFailed, "UNKNOWN_FIELD"):
            getattr(client, method_name)(Note, where={"nope": 1})
