...
def test_every_registry_collection_is_fingerprinted() -> None:
    ...
    allowed_unfingerprinted: set[str] = {
        # Upcasters are version-migration machinery, not a declared type.
        # Their effect on drift travels in `OntologyFingerprint.versions` and
        # is pinned by the version/upcaster tests in this file.
        #
        # Do not start a comment line with `# type:`: mypy reads it as a PEP
        # 484 type comment, fails to parse the prose as a type, and aborts the
        # whole run with `Invalid syntax` before checking anything.
        "upcasters",
    }
