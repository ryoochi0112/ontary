...
The ten tools are registered with these MCP safety hints:

| Tools | Annotation |
| --- | --- |
| `list_object_types`, `list_link_types`, `list_action_types`, `list_functions` | `readOnlyHint=True` |
| `get_declarations`, single-object reads, `query_objects`, `traverse_links`, `call_function` | `readOnlyHint=True` |
| `execute_action` | `destructiveHint=True` |

`query_objects` accepts `where`, `limit`, and `after`. Its `where` grammar is
equality-only matching on payload fields; unknown keys return the existing
`UNKNOWN_FIELD` error. The MCP server applies a default limit of 100 and refuses
